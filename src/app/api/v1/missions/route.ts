...
export async function POST(req: NextRequest) {
  try {
    ...
    // 2. Parse body and create mission via service
    const body = await req.json();
    const result = await MissionService.createMission(user.id, body);
...
export async function PUT(req: NextRequest) {
  try {
    ...
    // 2. Parse body and bulk reorder course missions
    const body = await req.json();
    const { courseId, orderedIds } = body;

    const result = await MissionService.reorderMissions(user.id, courseId, orderedIds);
