...
export async function PUT(
  ...
  try {
    ...
    // 2. Await dynamic parameter containing questionId
    const { id } = await params;
    const body = await req.json();

    // 3. Update question details via service
    const result = await QuizService.updateQuestion(user.id, id, body);
...
export async function DELETE(
  ...
  try {
    ...
    // 3. Delete question via service
    const result = await QuizService.deleteQuestion(user.id, id);
