...
export async function PUT(
  ...
  try {
    ...
    // 2. Await dynamic parameter containing answerId
    const { id } = await params;
    const body = await req.json();

    // 3. Update answer details via service
    const result = await QuizService.updateAnswer(user.id, id, body);
...
export async function DELETE(
  ...
  try {
    ...
    // 3. Delete answer option via service
    const result = await QuizService.deleteAnswer(user.id, id);
