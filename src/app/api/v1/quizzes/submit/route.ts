...
export async function POST(req: NextRequest) {
  try {
    ...
    // 2. Parse body and submit choices for grading
    const body = await req.json();
    const result = await QuizService.submitAttempt(user.id, body);
    ...
    // Returns exact response schema from api.md: { score, passed, xp }
