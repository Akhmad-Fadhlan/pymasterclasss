...
export async function POST(req: NextRequest) {
  try {
    ...
    // 2. Parse body and create slide via service
    const body = await req.json();
    const result = await SlideService.createSlide(user.id, body);
...
export async function PUT(req: NextRequest) {
  try {
    ...
    // 2. Parse body and bulk reorder lesson slides
    const body = await req.json();
    const { lessonId, orderedIds } = body;

    const result = await SlideService.reorderSlides(user.id, lessonId, orderedIds);
